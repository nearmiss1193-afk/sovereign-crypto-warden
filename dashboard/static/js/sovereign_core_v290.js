/* ── SOVEREIGN PRIME FOREX — DASHBOARD JS v2.9.0-DEPTH ── */
...
socket.on("full_update", data => {
  ...
  if (data.broker_status) renderBrokerUI(data.broker_status);
});

// Helper to render broker UI from data
function renderBrokerUI(tl) {
  ...
  const msgText = tl.configured
    ? "E8 Markets — Account " + (tl.account_id || "?") + " | Server: " + (tl.server || "E8") + " | DNS: " + (tl.dns_mode || "Native")
  ...
  if (badge) {
    badge.textContent = tl.configured ? "READY" : "NOT CONFIGURED";
    badge.style.color = tl.configured ? "var(--accent-green)" : "var(--accent-red)";
  }
}
...
function addSignalCard(sig) {
  ...
  card.innerHTML = `
    ...
    <div class="sig-meta">
      <span>Stop: ${sig.stop_pips || 0} pts</span>
      <span>Depth Imb: <small>${sig.l2_imbalance || 1.0}x</small></span>
